# PPL-harjoittelu – kehittäjän ohje

Tämä tiedosto sisältää projektin tekniset kehitys- ja ylläpito-ohjeet
(kysymysdatan muokkaus, PDF-muunnos, validointi, buildaus). Sovelluksen
käyttöohje ja käyttöehdot löytyvät tiedostosta `README.md`.
...
ppl-harjoittelu/
  ...
  README.md               # Käyttäjän ohje ja käyttöehdot
...
PDF:t eivät sisällä vastausavainta, joten kaikkien kysymysten oikea
...
`data/*.json` yhdeksi tiedostoksi `PPL-harjoittelu.html`. Skripti myös
kasvattaa `src/app.js`:n `APP_VERSION`-muuttujaa automaattisesti.
...
Kaikki sisältökorjaukset ja versiomuutokset kirjataan `CHANGELOG.md`-tiedostoon.
